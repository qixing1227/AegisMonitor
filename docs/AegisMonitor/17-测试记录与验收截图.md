...
| 3 | final-service-components.png | demo-web-01 的 NGINX、端口 80、RUNNING | 待最终答辩机器补录 |
| 4 | final-alert-open.png | 高 CPU 告警为 OPEN，ACK 按钮可见 | 待最终答辩机器补录 |
| 5 | final-alert-acked.png | 同一告警变为 ACKED，确认人可见 | 待最终答辩机器补录 |
...
核心监控闭环、59 项自动化测试和真实多机证据均已完成。关闭 Issue #4 前，还需按第 5 节补录服务组件、告警 OPEN、告警 ACKED 三张页面截图，并确认五张图片均可在 Markdown 中正常显示。
