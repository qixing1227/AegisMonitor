...
### 3.9 host_metric_points 主机历史指标表

用途：保存 CPU、内存和 TCP 连接数历史点，为详情页趋势图和区间查询提供数据。

| 字段 | 类型建议 | 说明 |
| --- | --- | --- |
| host_id | VARCHAR(64) | 主机 ID，联合主键 |
| reported_at | VARCHAR(64) | Agent 上报的 ISO-8601 时间 |
| reported_at_epoch_ms | BIGINT | 用于范围过滤和排序的毫秒时间戳，联合主键 |
| cpu_usage_percent | DOUBLE | CPU 使用率 |
| memory_usage_percent | DOUBLE | 内存使用率 |
| tcp_connection_count | INT | TCP 连接数 |

索引：`(host_id, reported_at_epoch_ms)`。相同主机、相同时间戳重复上报时执行幂等更新。
...
erDiagram
    ...
    hosts ||--o{ host_metric_points : reports
...
本节保留为生产规模演进设计。当前课程设计运行版本通过 `HostMetricRepository` 将历史指标写入 MySQL 的 `host_metric_points` 表；当主机规模和保留周期增长时，可新增 InfluxDB Repository 适配器而不改变 Controller 和前端接口。
