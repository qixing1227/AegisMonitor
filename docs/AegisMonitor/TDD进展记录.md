...
| Sprint 2 监控展示 | 已完成：主机列表、主机详情、历史趋势、服务组件、告警中心、自动刷新、Demo 数据入口 |
| Sprint 3 告警与权限 | 已完成告警 ACK 闭环；登录、RBAC、用户管理、告警规则编辑暂作为后续扩展 |
| Sprint 4 测试、演示与答辩准备 | 已完成核心交付：64 项测试、五张验收截图、答辩脚本、测试记录和五人分工；PPT 与课程报告待整理 |
...
### 3.50 Agent 上报认证与 API 错误状态加固

RED：

- 指标和服务上报接口虽然接收 X-Agent-Secret，但未校验凭证，错误密钥和跨主机上报仍返回 HTTP 200。
- 查询没有指标快照的主机时抛出未处理异常，表现为 HTTP 500。
- 错误响应缺少统一的 ApiResponse 结构。

GREEN：

- 在 AgentRegistry 集中实现 Agent ID、Secret 与 Host ID 的认证授权，心跳、指标和服务上报复用同一规则。
- 无效或未知凭证返回 HTTP 401，合法 Agent 跨主机上报返回 HTTP 403。
- 新增全局 ApiExceptionHandler，统一返回 success=false、HTTP 状态码名称、错误消息和空数据。
- 不存在的主机指标快照返回 HTTP 404，非法历史范围保持 HTTP 400。
- 错误消息不回显 Agent Secret，也不向调用方暴露内部堆栈。

验证：

- backend\test.cmd：26 项通过。
- agent\test.cmd：18 项通过。
- frontend npm test：20 项通过。
- frontend npm run build：通过。
- 全项目共 64 项自动化测试通过。

结论：

- 已 GREEN，代码可进入最终交付冻结阶段。
...
| Sprint 4 测试、演示与答辩 | 高 | 64 项测试、五张验收截图、答辩脚本、测试记录和五人分工已完成；PPT、报告和彩排待收口 |

下一阶段建议进入“团队协作完善与答辩收口”，而不是继续补 Sprint 1：

1. 组长提交并合并历史指标与趋势图分支。
2. 队员 4 在最终答辩机器补录服务组件和告警 ACK 前后截图。
3. 全组按五人分工完成一次 10 分钟计时彩排。
4. 组长根据最终文档制作课程报告和 PPT。
5. 合并最终交付 PR，关闭 Issue #4，并创建 v1.0.0 标签。

暂不建议在当前阶段扩展完整登录、RBAC、服务拓扑、企业微信/邮件通知或云服务器部署。历史曲线已经进入可运行代码；其后续演进重点是按数据保留周期清理和可选 InfluxDB Repository，而不是再次改动前端接口。

## 2026-06-27 课题二要求补齐审查

- 对照课题二原文完成符合性审查，新增 `docs/AegisMonitor/20-课题二要求符合性审查.md`。
- 补齐主机利用率中的磁盘与网卡指标：后端持久化 `disk_usage_percent`、`network_bytes_sent`、`network_bytes_received`，前端详情页和历史曲线可展示。
- 补齐服务组件层指标：Agent 服务发现上报进程 CPU 与内存 RSS，后端持久化，服务组件页展示 `CPU / 内存`。
- 验证结果：Agent 18 项、后端 29 项、前端 21 项全部通过，前端生产构建通过。
